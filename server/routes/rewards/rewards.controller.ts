...
export class RewardsController {
  ...
  async createOne(
    ...
    if (project) return await this.rewardsService.createOne(projectId, createRewardDto);
  ...
  @Roles(Role.User, Role.Admin)
  @Get(":projectId")
  async findAll(@Param("projectId") projectId: number, @Req() req: Request): Promise<Reward[]> {
    const project = await this.projectsService.verifyUserProject(Number(req.user.id), projectId);
    if (project) return await this.rewardsService.findAll(projectId);
    else throw new Error("You are not the owner of the project");
  }
